...
- **Currency is stated as a tree state rather than a date.** This inventory reflects the repository
  as this commit publishes it — the state after the OWASP Top 10 (2021) security audit and
  remediation. A dated "As of" line goes stale the moment the next edit lands — evidence gathered in
  August sitting beneath a July currency line asserts a freshness the content no longer has. Reproduce
  the inventory against any
  checkout with the parsing command named in the paragraph above; the manifests, not a date, are the
  authority. Four dependency versions changed as part of that work; every changed row is marked in
  the table below and explained under *Dependency changes made by the security remediation*.
...
> product declares to third parties. There is no patched release under permissive terms to retreat
> to, and the reversal path would require a repository-wide `NU1903` suppression that the CI
...
- **Every project must be covered, and coverage is complete at 19 of 19 - by two routes, not one.** `dotnet sln WebVella.ERP3.sln list` returns **17** projects of the **19** tracked `.csproj` files. The remaining two, `WebVella.Erp.WebAssembly/Server` and `WebVella.Erp.WebAssembly/Shared`, are deliberately **not** solution members and are covered by their own named restore, build and advisory steps, whose output is appended to the same evidence files. So coverage is 19 of 19 while the *solution* is 17 of 19, and the two numbers are not interchangeable - the two are not interchangeable.

  **Why the solution is not simply widened to 19.** The frozen plan authorises exactly one change to `WebVella.ERP3.sln`: the H-19 project-path casing correction (plan section 0.7.1, group 1). Altering solution membership is not a security fix, so enrolling further projects is out of scope however convenient a single-command scan would be. Enrolling them is not permitted and is not the model; the plan authorises one change to the solution file, so the enrollment is absented, and the solution file's diff against its base is now exactly the one authorised casing line. Non-membership is a scope boundary, not a coverage gap: `Directory.Build.props` is *directory*-scoped, so both projects inherit the audit and analyzer settings identically, and their dedicated build step asserts each resolved `TargetFramework` by name - which is how finding **H-18**, the end-of-life `net7.0` line both of them once sat on, is re-proved on every push rather than by out-of-band verification.
...
| 17 solution members | `dotnet build WebVella.ERP3.sln -c Debug --no-restore -t:Rebuild` | exit 0, **0 errors**, **3,055** warnings, with all 17 members confirmed present in the log. Nothing is suppressed to reach that figure: the one analyzer property the remediation moved narrowed the `CA3001`-`CA3012` `NoWarn` from every project to `WebVella.Erp.Web` alone, which arms more rules rather than fewer, and no `#pragma` and no `SuppressMessage` attribute was added anywhere in the range, which is checkable with `git diff 551a9437 a3e9c8ae -- '*.cs' '*.csproj' Directory.Build.props \| grep -E '^[+-].*(NoWarn\|#pragma\|SuppressMessage)'`. The count therefore reflects code the remediation removed or rewrote, and the security half is held to its recorded baseline independently: Gate 1 asserts the 55-diagnostic security baseline has not grown and that the distinct `(rule, file)` pairs match the reviewed allow-list exactly. Measured rather than assumed for the two projects outside the solution: they emit no diagnostic of their own, and the Client project their build also compiles remains a solution member |
...
Enrolling both is not the model, and the 17-of-19 graph is not a gap that had been "closed properly". Review finding `CR2-F-06` rejected that: the frozen plan authorises exactly one change to the solution file, the H-19 casing correction, so enrolling them would be scope drift and the 17+2 split is the intended model rather than a shortfall. Membership is asserted on every run by `Assert solution membership matches the declared coverage model`, which fails if a tracked project is in neither the solution nor the declared non-member list, if a project is in both, or if a declared project is missing from the tree.
...
- **Gate 1, the analyzer ratchet, had never been able to pass at all.** MSBuild compiles projects on parallel nodes and prefixes each diagnostic line with the node id, so the parse captured `    17>` and the leading indentation *into the file field*. Nothing could then equal its allow-list entry, and `comm` declared all 42 security diagnostics unreviewed and all 21 accepted residuals stale in the same breath. Stripping the node prefix reduces the parse to 21 pairs matching the allow-list exactly - 0 unreviewed, 0 stale - and collapses per-node duplicates, taking the all-category count down to the distinct `(rule, file)` pairs the gate actually compares - **641** distinct `(rule, file)` pairs from **6,056** raw CA diagnostic lines, which reduce to **3,028** unique `(file, line, column, rule)` diagnostics. A fail-closed guard now stops the job if that normalisation ever regresses, because the symptom of the bug was a gate that could not pass however clean the tree was.
...
| `dotnet build … --no-restore --no-incremental` | `Build succeeded.` **0 errors**, **3,055** warnings; every diagnostic a project-level warning, none promoted |
...
| Gate 2 - `dotnet list package --vulnerable --include-transitive` | 19 of 19 projects report `has no vulnerable packages`, and no row matches `High` or `Critical` - the **17** solution members through the solution-wide listing and the **2** declared non-members through their own, so the total is reached by two routes. The listing now additionally asserts that every solution member actually appeared in its output, which is what makes the count non-vacuous; see *The authoritative dependency result at this revision* above |
...
The **twenty-three** artifacts that run retained - and that the workflow uploads, with an explicit
thirty-day retention rather than an inherited default - are `analyzer-build-summary.txt`,
`history-exposure-commits.txt`, `security-rule-ids.txt`, `all-ca-diagnostics.txt`,
`security-diagnostics.txt`, `security-allowlist.txt`, `security-allowlist-justified.txt`,
`security-unreviewed.txt`, `taint-scan-web.txt`, `header-compliance.txt`, `positive-control.txt`,
`sln-members.txt`, `tracked-projects.txt`, `declared-non-members.txt`, `extra-projects.txt`,
`vulnerable-packages.txt`, `vulnerable-packages-extra.txt`, `vulnerable-packages-wasm.txt`,
`secret-sweep.txt`, `startup-smoke.txt`, `negative-control.txt`, `editorconfig-items.txt` and
`manual-verification-matrix.txt`. **The three newest are the ones code-review findings `MAJ-01` and
`MAJ-03` added**: the justified form of the Gate 1 allow-list, so a reader sees each accepted
diagnostic's risk reference and disposition rather than only the tolerance; the terminating
`CA3001`-`CA3012` scan of the one compilation the ordinary build excludes; and the
mandated-header-name compliance verdict. **The count is not restated in prose here** because the workflow
derives it, so the sentence and the list are checked against each other by the evidence-completeness step
rather than by a reader. The raw log IS withheld, deliberately and under
its real name `analyzer-build.txt`: it carries roughly 3,100 diagnostics each stamped with an
absolute runner path, so publishing it would broaden internal-path exposure (CWE-532) and bury the
security signal in the pre-existing style backlog. What is published in its place is the derived
`analyzer-build-summary.txt`, which Gate 1 asserts contains no path separator before it is uploaded -
the property that makes it safe to publish - while the raw log is still written locally and is
still what every assertion in that step parses, so no evidence is lost. Any failure-log tail the job
prints is passed through a self-tested redactor first, so a publish cannot leak the material the
gate exists to police. They are written into the workspace at run time and are deliberately **not**
committed - `.gitignore` enforces that rather than leaving it to habit - because a committed copy
would be a stale record vouching for commits it never saw, which is the failure mode this whole
section exists to avoid. One of them, `manual-verification-matrix.txt`, stamps the run identity into
its own header (`generated`, `commit`, `ref`, `run`, `release ctx`), so the citation above is
traceable to a machine-written record rather than resting on this prose. **A further path has since
been added to the upload — the twenty-fourth at this revision — and it is different in kind:**
`evidence-manifest.txt` is written by the step `Assert the scan evidence is complete and bind it to
the commit`, which refuses to let the publish proceed while any of the twenty-three is absent or
empty, digests each one with SHA-256, and records the commit the digests belong to. Before it
existed the publish would happily upload nothing and still be green. At this revision that step
reports `Evidence complete - 23 file(s) present, each digested and bound to commit` — twenty-three
evidence files plus the manifest that vouches for them, hence **24** published paths against a
required set of twenty-three. **The count in that message is now DERIVED from the manifest rather
than written into the step**, because the previous revision hard-coded "20 files" and the required
set grew to 23 while the sentence did not — the same drift review findings `MAJ-06` and `MAJ-07`
report across this documentation set, and a figure that is computed cannot repeat it.
...
**The enumeration below is complete against the workflow matrix, `M19` included** — the scenario covering the WebAssembly logout seam. A coverage audit run from this document therefore sees every row the matrix declares.
...
| `WarningsAsErrors` | appends `NU1900;NU1901;NU1902;NU1903;NU1904;NU1905` | Turns any advisory diagnostic into a **build error**. **Six** codes, not four: the four severity codes `NU1901`-`NU1904`, plus the two data-availability codes `NU1900` and `NU1905`. Including those two is the load-bearing part - without them, an audit that cannot reach the advisory database reports a *warning* and the build goes green, which is a gate that fails open exactly when it matters. The value appends to `$(WarningsAsErrors)` rather than replacing it, so nothing already set is discarded - the .NET SDK's own escalations (`SYSLIB0011`, `NU1605`) are still appended after these six and survive intact. This is the only place in the repository that sets the property. |
| `EnableNETAnalyzers` / `AnalysisLevel` | `true` / `latest-recommended` | Enables the built-in .NET analyzers, which stand in for an external static-analysis scanner. The general categories run at the recommended set, and analyzer diagnostics stay **warnings** at the project level - only the six dependency codes above are build errors. Four Security-category rules are active at this level alone and are measured, ratcheted and positively controlled by the workflow: `CA5350` and `CA5351` (weak and broken cryptographic algorithms), `CA5359` (certificate validation disabled) and `CA5364` (deprecated security protocols). Measured repository counts are `CA5350` 0, `CA5351` **5**, `CA5359` 0, `CA5364` 0. |
| `AnalysisLevelSecurity` | `latest-all` | Raises the **Security category only** to the SDK's shipped `all` configuration, which is what arms the families Validation Gate 1 names by name and that the general level leaves off: query construction (`CA2100`), unrestricted type-name handling (`CA2326`-`CA2328`), hard-coded keys (`CA5390`), non-random initialisation vectors (`CA5401`) and disabled token-validation checks (`CA5404`). The **level** suffix is the only form that works - `AnalysisLevelSecurity` defaults to `AnalysisLevel`, so the SDK target that selects the category's configuration reads `recommended` from it and never consults `AnalysisModeSecurity`. Arming is not escalation: nothing here promotes a `CA` rule to an error, and the workflow's Gate 1 ratchets each armed family against a measured baseline instead. `CA5382`, `CA5383` and `CA5402` are armed but have never been made to fire by any probe in this environment, so no baseline is claimed for them and the gap is recorded in the risk register. |
| `NoWarn` for `CA3001`-`CA3012`, one project | *(conditional)* | Excludes the interprocedural taint-dataflow family from **`WebVella.Erp.Web` only**, for a measured termination reason rather than a scope preference: armed and unbounded that compilation - 395 Razor views in one compilation - does not terminate, while each of the other eighteen projects completes in seconds reporting zero. The excluded compilation is still scanned, separately and under a cost bound, by its own workflow step, whose timeout is a hard failure so that an unfinished scan can never read as clean. The residual is depth rather than coverage and is carried as `RISK-051`. |
| No global analyzer config | *(deliberately absent)* | Neither a repository-root `.globalconfig` nor a `DiscoverGlobalAnalyzerConfigFiles` value nor an `EditorConfigFiles` item exists, and none may be added. Such a file is the one mechanism that can change any rule's **severity** for all 19 projects at once, and per-rule escalation stays outside this gate's scope; the category level above delivers the required coverage through a configuration the SDK ships and versions instead. The workflow asserts the absence over the tracked **and** untracked tree on every run, and separately asserts which configuration the compiler actually loaded, so neither half can drift unobserved. |
...
- **No advisory diagnostic is suppressed.** All six `NU19xx` codes are promoted to errors and none is silenced, in `Directory.Build.props` or in any project manifest. What *is* excluded is a single analyzer family in a single project - the `CA3001`-`CA3012` taint rules for `WebVella.Erp.Web`, for the termination reason in the table above - so the element-form count is one rather than zero, and that one hit is not an audit suppression. Measure it in the two ways that distinguish the cases:
    ...
    # 2. The effective NoWarn any project inherits carries no NU code, so no
    #    advisory is silenced. It is NOT uniform across projects - the core
    #    library inherits the SDK default, the excluded compilation the family:
    dotnet msbuild WebVella.Erp/WebVella.Erp.csproj -getProperty:NoWarn
    #   1701;1702
    dotnet msbuild WebVella.Erp.Web/WebVella.Erp.Web.csproj -getProperty:NoWarn
    ...
    # 3. The audit-suppression item list is empty for every project, which is the
    #    property that actually matters:
    dotnet msbuild WebVella.Erp/WebVella.Erp.csproj -getItem:NuGetAuditSuppress   # []

    # 4. No project manifest suppresses anything either. Driven by `git ls-files`
    #    rather than a bare recursive grep, so it covers exactly the 19 tracked
    #    manifests and cannot be perturbed by an untracked probe project:
    git ls-files -z '*.csproj' | xargs -0 grep -nE 'NuGetAuditSuppress|NoWarn'   # no matches
    ```

  Accepting an advisory rather than clearing it is a governed act: it requires a recorded, justified risk acceptance in the [risk register](https://github.com/Blitzy-Sandbox/blitzy-WebVella-ERP/blob/master/docs/security/risk-register.md) naming the advisory, its severity, why the upgrade is declined and who accepted the residual risk, and it must name one narrowly scoped diagnostic rather than a wildcard. `RISK-001` carries the one open case.
...
# AnalysisLevelSecurity raises the Security category and DiscoverGlobalAnalyzerConfigFiles is
# deliberately unset. Assert both values rather than assuming them: a change to either moves the
# rule set away from the level every baseline and allow-list entry here was measured at.
...
# And assert that no REPOSITORY-supplied global analyzer config reaches the compiler - an untracked
# file at the root, or an explicit EditorConfigFiles item, would both show up here. The SDK's own
# analysislevel_<n>_recommended and analysislevelsecurity_<n>_all configurations are expected and
# are what the category widening selects; anything from this tree is not. Same check the workflow
# performs on every run, in both directions.
...
A dependency inventory that omits the cryptography and token-handling libraries the platform actually loads is not an adequate basis for a security review, even when none of them carries an advisory. Every transitive package below is therefore either security-relevant by function, changed by this remediation, or resolved at a version that differs from another copy of itself in the same solution.
...
**The version is recoverable from the binary itself.** It carries Windows version resources, and three independent sources inside it agree on `0.12.4`:
...
- **How the advisory is closed:** by the version change itself, not by suppressing the detector. `dotnet restore WebVella.ERP3.sln` completes with zero `NU19xx` diagnostics and `dotnet list WebVella.ERP3.sln package --vulnerable --include-transitive` reports no vulnerable packages. **`Directory.Build.props` declares no `NuGetAuditSuppress` and no `WarningsNotAsErrors`, and its one `NoWarn` carries the `CA3001`-`CA3012` taint family for a single project rather than any advisory code.** Verify it as the gate's own bullet above does: strip the XML comments and only that family remains, and `-getItem:NuGetAuditSuppress` returns an empty list for every project.
...
**Where a suppression would have to be declared, if the owner ever decides `RISK-001` the other way.** Repository-wide, in `Directory.Build.props`, rather than in the core manifest - and that is a necessity of how the audit is scoped rather than a widening of scope. NuGet audit runs **per project**, and `WebVella.Erp` is referenced by every other project, so AutoMapper is present in 16 project graphs transitively: while the vulnerable version was still pinned, a project-scoped suppression in `WebVella.Erp.csproj` silenced only that one project and the solution restore still failed, measured at exactly 15 `NU1903` errors, one per remaining affected project. The correct instrument is a single `NuGetAuditSuppress` item naming that one advisory by URL, never a wildcard and never a `NoWarn` on the `NU19xx` codes, which would silence every future advisory too. **Nothing of the kind exists today**: the pin is at `[15.1.3]`, `-getItem:NuGetAuditSuppress` resolves to `[]` for every project, and the only live `NoWarn` any project inherits is the `CA3001`-`CA3012` taint family.
...
- **The committed native binary was inspected directly rather than assumed opaque.** `ExternalLibraries/libwkhtmltox.dll` carries Windows version resources identifying it as version `0.12.4` and embeds LGPL v3 licence text, so its row and the *Committed native binaries* section record the version as a fact with the reproduction commands beside it. What remains genuinely unrecorded is the artefact's **provenance and its applicable licence** - there is no manifest, no checksum, no upstream reference and no licence file committed with it - so those, and only those, are marked unverified.
...
- [Security audit report](docs/security/security-audit-report.md) - findings in the mandated eight-field format (FINDING, SEVERITY, CWE, LOCATION, DESCRIPTION, IMPACT, EVIDENCE, REMEDIATION). It carries a full eight-field record for **all 53** AAP findings, one per identifier in the ranges `C-01`-`C-05`, `H-01`-`H-20`, `M-01`-`M-18` and `L-01`-`L-10`, and **138** such records in total once the post-remediation review inventories in Parts 2 through 5 are included — 53 in Part 1, 20 in Part 2, 26 in Part 3, 16 in Part 4 and 23 in Part 5. *(The workflow asserts a floor of 138 and that all 138 identifiers are distinct.)* Every one of the 53 has its own record, so nothing is summarised by band. Reproduce both figures with `grep -c '^| \*\*FINDING\*\*' docs/security/security-audit-report.md` for the total and `grep -cP '^#### (C|H|M|L)-[0-9]{2} ' docs/security/security-audit-report.md` for the Part 1 per-finding count — which is the pattern the workflow's *Assert every audit-report finding identifier is unique* step uses, so this row and the gate cannot disagree. *(Do not substitute `grep -cE '^#{1,6} [CHML]-[0-9]+ — '`: it returns **70** rather than 53, because it also matches Part 2's single-digit identifiers `H-1`–`H-9`, `M-1`–`M-7` and `L-1`. The two-digit anchor is what confines the count to the zero-padded Part 1 inventory.)*
