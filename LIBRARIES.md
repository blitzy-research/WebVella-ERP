...
These commands are not left to be run by hand. `.github/workflows/security-scan.yml` runs every one of them, so the coverage described here is reproducible rather than a one-off local observation. Two honest qualifications on that. First, the workflow does not restate the SDK version: it resolves the toolchain with `global-json-file: global.json`, so the pin has exactly one home and cannot drift out of step with the build. Second, its steps were validated by parsing the YAML and executing each `run:` step in order against this working tree under `bash`, which is the shell GitHub actually uses for a `run:` block. The file declares **eighteen** steps, of which three are `uses:` actions rather than shell, leaving **fifteen** shell steps. Fourteen of the fifteen are the evidence-gathering gates, and all fourteen were executed and **all fourteen exited 0**, in a single ordered pass whose per-step exit codes and durations were recorded.

The fifteenth shell step is different in kind and must not be counted alongside them. `Release gate - every mandatory manual scenario must be attested` was added in response to review finding `OBS-07`, and it is **expected to exit non-zero on this tree**, because no manual attestation has been committed. That is the step working, not failing: before it existed, all nineteen manual scenarios could sit unproved while the job reported success, so a green workflow did not mean what a reader would reasonably take it to mean. It is reported here as red-by-design rather than folded into the "all exited 0" count, because a document that averaged the two would recreate exactly the ambiguity the finding identified. The remedy is to execute the scenarios against a deployment and commit `manual-verification-results.txt`; fabricating that file would defeat the only purpose the gate has.
...
> evidence for the state it measured. Two of its figures no longer describe this revision: the workflow held
> **sixteen** steps then and holds **seventeen** now, of which **fourteen** are `run:` blocks; and the Gate 5
> matrix declared **32** rows then and declares **33** now. One of its verdicts is also no longer the reported
> status: Gate 5 is now recorded as **DEFERRED** rather than passed, because one mandatory scenario, `M16`,
> has not been executed. Current figures and the gate-by-gate status for this revision are stated once, each
> with the command that produces it, in
...
That sequence was executed in workflow order against the tree this commit publishes, completing at **2026-08-04T05:39:57Z** - the timestamp the last step stamps into `manual-verification-matrix.txt`, so the run is anchored to a machine-written record rather than to this sentence. An earlier pass of the same sequence completed at 2026-08-04T00:50:03Z against a partial tree; the later timestamp is the one that describes what this commit contains, and it is stated rather than the earlier one because a gate record must name the tree it actually read. It ran against the working tree at parent commit `daf4aa90465bfad856cf3afb4540d147b7026ded` plus the modifications this commit introduces - a gate run necessarily predates the commit it validates, so the hash the artifacts recorded is the parent's rather than this commit's. **All 14 evidence-gathering shell steps exited 0, and the aggregate exit status across those fourteen was 0.** The fifteenth shell step, the release gate described above, did not exist when this pass ran and is red by design until manual attestations are committed, so it is excluded from that aggregate rather than silently absorbed into it. Every figure in the table below was read back out of the evidence artifact its own step wrote during that run - none of it is restated from an earlier draft, and where a step no longer exists its row has been removed rather than left standing:

| Step | Recorded result of the 2026-08-04T05:39:57Z run |
|------|--------|
...
| Gate 2 - `dotnet list package --vulnerable --include-transitive` | 19 of 19 projects report `has no vulnerable packages`, and no row matches `High` or `Critical` - the **17** solution members through the solution-wide listing and the **2** declared non-members through their own, so the total is reached by two routes |
...
| Gate 5 - AAP 0.9.1 verification matrix | **41 rows: 16 proven, 25 deferred, 0 failed** - the row count grew from 32 as six scenarios were added for controls that had none and two evidence-derived rows were added for the new analyzer controls. Each proven row cites the required lines it found in a named artifact from this same run. Each deferred row needs a live PostgreSQL instance, a browser or an SMTP server, so the job records it DEFERRED and prints the procedure and its binding prerequisites rather than claiming a pass. **24 of the 25 are classified REQUIRED and are release-blocking**: they are reported on an ordinary run and are fatal on a version-tag push or a release-candidate dispatch, and the matrix prints `RELEASE-READY=no` on every run while any of them is unproven, so a green ordinary run cannot be read as release readiness |
