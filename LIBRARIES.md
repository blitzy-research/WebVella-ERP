...
| 17 solution members | `dotnet build WebVella.ERP3.sln -c Debug --no-restore -t:Rebuild` | exit 0, **0 errors**, 3,044 warnings - the figure read 3,096 while a repository-root `.globalconfig` armed additional rules, and was re-measured once that file was removed. Unchanged by the reversion from 19 members to 17, measured rather than assumed: the two removed projects emit no diagnostic of their own, and the Client project their build also compiles remains a solution member |
...
| `dotnet build … --no-restore --no-incremental` | `Build succeeded.` **0 errors**, **3,044** warnings; every diagnostic a project-level warning, none promoted |
...
| Gate 1 positive control | a throwaway project containing a deliberate weak hash, a disabled certificate-validation callback and a deprecated transport protocol was compiled, and `CA5350`, `CA5359` and `CA5364` were each asserted present in the build log - proving the analyzers report defects rather than merely running. The control asserts **both directions**: those three MUST fire, and `CA5390` and `CA2100` MUST NOT, because they do not execute at `latest-recommended` and a run in which they suddenly did would mean the analysis level had silently widened and every baseline here had gone stale. All are warnings, not errors, so the probe build exits zero; the assertion reads the LOG rather than the exit status either way. Measured detail worth keeping: `CA5359` fires on `ServicePointManager.ServerCertificateValidationCallback` but **not** on `HttpClientHandler.ServerCertificateCustomValidationCallback`, so the probe uses the former. |
...
| Gate 5 - AAP 0.9.1 verification matrix | **32 rows: 0 failed.** Each proven row cites the required lines it found in a named artifact from this same run; each deferred row needs a live PostgreSQL instance, a browser or an SMTP server, so the job records it DEFERRED and prints the procedure rather than claiming a pass |
...
The twelve artifacts that run retained - and that the workflow uploads - are `analyzer-build.txt`, `security-rule-ids.txt`, `all-ca-diagnostics.txt`, `security-diagnostics.txt`, `security-allowlist.txt`, `security-unreviewed.txt`, `positive-control.txt`, `vulnerable-packages.txt`, `secret-sweep.txt`, `startup-smoke.txt`, `negative-control.txt` and `manual-verification-matrix.txt`. They are written into the workspace at run time and are deliberately **not** committed - `.gitignore` enforces that rather than leaving it to habit - because a committed copy would be a stale record vouching for commits it never saw, which is the failure mode this whole section exists to avoid. One of the twelve, `manual-verification-matrix.txt`, stamps the run identity into its own header (`generated`, `commit`, `ref`, `run`), so the citation above is traceable to a machine-written record rather than resting on this prose.
...
Gate 5 defers eighteen rows because a workflow job has no database, browser or mail server. Sixteen of the eighteen were nevertheless exercised by hand on this machine, against a published host running over HTTPS with a live PostgreSQL 16 instance and a real browser session, on 2026-08-03/04 UTC at the same commit. That work is recorded here because an unexecuted row and an executed one must not read alike - and because the gate itself still reports them DEFERRED, since an attestation is a statement about one deployment at one moment and is not committed for the same reason the artifacts are not.
...
Exercised in part, with the unexecuted half named rather than absorbed - five rows:
...
* **It contains no active suppressions.** No advisory diagnostic is silenced, in this file or in any project manifest. One caveat keeps the claim honest: a plain `grep -nE 'NoWarn|WarningsNotAsErrors|NuGetAuditSuppress' Directory.Build.props` returns **one** line - `<NoWarn>$(NoWarn);NU1903</NoWarn>` at `Directory.Build.props:L457` - which sits **inside an XML comment**. It is a documented, deliberately inert *suppression seam*: the only sanctioned shape a future suppression may take, one narrowly scoped code, and the surrounding comment requires a named risk acceptance in `docs/security/risk-register.md` before it may ever be uncommented. Three measurements establish that nothing is suppressed today:
...
* **How the advisory is closed:** by the version change itself, not by suppressing the detector. `dotnet restore WebVella.ERP3.sln` completes with zero `NU19xx` diagnostics and `dotnet list WebVella.ERP3.sln package --vulnerable --include-transitive` reports no vulnerable packages. **`Directory.Build.props` declares no `NuGetAuditSuppress` and no active `NoWarn` or `WarningsNotAsErrors`.** An earlier revision of this bullet said `grep -nE 'NoWarn|WarningsNotAsErrors|NuGetAuditSuppress' Directory.Build.props` "returns nothing", which was never true: it returns exactly one line, the commented-out suppression seam. Verify it the way the gate's own bullet above does - strip the XML comments first, and then nothing matches at all.
