...
#region << ***Update entity*** Entity name: role >>
{
	...
	// THREAT ADDRESSED - finding C-05, CWE-269 (improper privilege management) and CWE-732 (incorrect
	// permission assignment for critical resource), OWASP A01: Broken Access Control. EntityManager
	// .UpdateEntity REPLACES the entire record-permission set rather than merging into it, so the
	// anonymous Guest CREATE grant this patch used to re-add here silently overwrote the revocation
	// performed both by the core seed and by the schema version 4 migration. Plugin patches run after
	// core provisioning, so every deployment that loads this plugin had anonymous role creation
	// restored - which is privilege escalation, because a role is the unit that carries permissions.
	// The grant is therefore removed here too.
	//
