...
| Dependency gate | `dotnet list WebVella.ERP3.sln package --vulnerable --include-transitive` reports **no vulnerable package in any of the 17 solution projects**, `AutoMapper` resolving at `15.1.3`. The two non-member WebAssembly projects were listed separately and are also clean, so the repository-wide result is clean across all 19 — but it takes **three** commands, not one |
| Build gate reach | All six gate properties evaluate on **19 of 19** projects from `Directory.Build.props`, including the two that are not solution members — the props file is directory-scoped, so gate inheritance does not depend on solution membership |
...
| Still open, and deliberately so | `AllowAnyOrigin()` remains at two hosts (`RISK-013`); the **6-to-24-character password bounds** and the **guest-role create grants** in `WebVella.Erp/ERPService.cs` are unchanged (findings M-13, C-02 and C-05). These belong to the cross-origin and authorization classes, which are **not** part of this change. One demo credential also remains in the Blazor WebAssembly **client** page `Client/Pages/Index.razor.cs`, which is outside this change's authorised file set. Nothing in this report should be read as claiming otherwise — see the [risk register](risk-register.md) |
...
| **EVIDENCE** | The concatenations are present verbatim; the analyzer gate reports **zero** `CA2100` and zero `CA23xx` diagnostics across the 17 solution projects and, built separately, the two non-member WebAssembly projects — 19 in total — corroborating that values are parameterised and the exposure is identifier-only. |
...
| **REMEDIATION** | The SDK is pinned to `10.0.302` with `rollForward: disable` — the strictest of the available policies, chosen over `latestPatch` because the weaker setting still lets the patch level drift and **both** halves of the gate (the audit defaults and the analyzer rule set) are SDK-version dependent, so patch drift alone is enough to change a gate result. The accepted cost is that the exact SDK must be installed; the failure is loud and names the required version. **The lock-file half is deliberately not done**: adding one changes restore behaviour for every project and every contributor, which is materially more invasive than the finding warrants, and the exact-version pins already present on the two most security-relevant packages give the same guarantee where it matters most. Recorded as remaining guidance rather than silently dropped. |
...
| **REMEDIATION** | **Partly fixed, and the remainder is disclosed rather than closed — this entry was corrected after the fact.** *The framework half is fixed:* both projects are retargeted to `net10.0`, so no project remains on an end-of-life framework and finding H-18 is closed. *The gate half was never actually broken:* `Directory.Build.props` is **directory-scoped, not solution-scoped**, so both projects inherit all six gate properties regardless of membership — verified by evaluating `NuGetAudit`, `NuGetAuditMode`, `NuGetAuditLevel`, `WarningsAsErrors`, `EnableNETAnalyzers` and `AnalysisLevel` on each of them directly. *The membership half is deliberately **not** fixed:* both projects were briefly enrolled in `WebVella.ERP3.sln`, and that enrolment has been **reverted**. Changing the solution's project membership is not a security fix and was outside the authorised change to that file, which is the project-reference path casing repair (H-19) and nothing else. `dotnet sln list` therefore returns **17**, as it always has. What remains is a **coverage** obligation rather than a vulnerability: the two WebAssembly projects must be restored, built and audited with explicit per-project commands. Both are clean when so audited, and the obligation is disclosed in the CI workflow's own coverage note, in the [remediation log](remediation-log.md) and in the [secure configuration guide](secure-configuration.md), so that no green solution-level run is mistaken for repository-wide coverage. |
