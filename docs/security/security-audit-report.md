...
| --- | --- | --- | --- |
| 1 | SAST scan: 0 Critical, 0 High | **PARTIAL, and materially less partial than it was** | The named external scanner could not be installed. The substitute is the .NET analyzers with `AnalysisLevel=latest-recommended` **and `AnalysisLevelSecurity=latest-all`**, which arms the whole Security category. Under code-review finding `GATE-03` the `CA3001`–`CA3012` taint exclusion was narrowed from all nineteen projects to `WebVella.Erp.Web` alone: measured per project, eighteen build in 0 to 6 seconds each with **zero** `CA3001`–`CA3012` diagnostics, while that one project is killed at a 600-second bound. A probe carrying deliberate taint flows reports `CA3001` and `CA3003`, and the workflow's positive control now **requires** both, so the eighteen zeros are an absence of defects rather than an absence of analysis. Re-measured at this revision: the armed build completes with **zero errors** and reports **55** distinct Security-category diagnostics across **five** rules — `CA2100` (20), `CA2326` (20), `CA2328` (9), `CA5351` (5) and `CA5362` (1) — which collapse to **21** `(rule, file)` pairs, and the ratchet's set difference against the reviewed-residual allow-list is **empty in both directions: zero unreviewed and zero stale**. So the gate's own criterion, *no unreviewed Security-category diagnostic*, is met exactly. It is still reported PARTIAL rather than PASS against the engagement's literal wording, for two reasons that are stated rather than absorbed: one project remains outside the taint family, and the five `CA5351` residuals are the legacy MD5 **verification** path that the credential migration requires — removing it would lock out every user whose hash has not yet been upgraded, which the preservation requirement forbids. See the Gate 1 detail below and `RISK-051`. |
...
| Gate 5 declared rows | **48** — `A01`–`A16` plus `M01`–`M32`; **31** of the 32 manual rows are REQUIRED and one, `M18`, is ADVISORY. All 32 hold a committed attestation, so the matrix reports `deferred=0` | `grep -n 'expected_rows=' .github/workflows/security-scan.yml`; `grep -c '^M' manual-verification-results.txt` |
...
then 16/3/13, then 17/3/14, now **23/3/20**), the Gate 5 row total (from 32, then 33, then 41, then 45, to **48**),
