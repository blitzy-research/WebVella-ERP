...
| --- | --- | --- | --- |
| 1 | SAST scan: 0 Critical, 0 High | **COVERAGE CLOSED at 19 of 19 compilations; the residual treatment is now formal and exit-bounded, with owner ratification pending** | The named external scanner could not be installed. The substitute is the .NET analyzers with `AnalysisLevel=latest-recommended` **and `AnalysisLevelSecurity=latest-all`**, which arms the whole Security category. **Code-review finding `MAJ-01` rejected the position this row previously took**, and rightly: it reported the `CA3001`–`CA3012` taint family running for eighteen of nineteen projects and treated the nineteenth — `WebVella.Erp.Web`, the controller and Razor view surface, and therefore the likeliest home of a taint flow — as covered by manual review. An unscanned compilation and a clean one are indistinguishable from outside, so that was not coverage. Both halves of `MAJ-01` are now supplied. **(a) A terminating scan of the excluded compilation exists and its result is retained as evidence.** Measured: armed with no cost bound that project runs past **2,700 seconds** without completing — a larger bound than the original 600-second observation, same outcome — while armed with the interprocedural chain bounded to **one hop** it completes in about **220 seconds**, exit 0, reporting **zero** `CA3001`–`CA3012` diagnostics. The workflow step *Gate 1 - terminating taint scan of the excluded compilation* performs it on every run, writes `taint-scan-web.txt` into the published evidence set, and — the load-bearing part — first applies the same bound to a probe carrying two deliberate taint flows and **requires** `CA3001` and `CA3003` to report, so the project's zero is the absence of a detected flow rather than the silence of a narrowed analysis. A timeout is a hard failure, never a pass. The cost configuration is written outside the repository and asserted to carry no per-rule severity, so the `.globalconfig` prohibition and the no-escalation boundary both stay intact. **What remains a residual is DEPTH, not coverage:** in that one project a flow longer than one interprocedural hop is not followed — carried as `RISK-051`. **(b) The five `CA5351` residuals now carry a formally approved, attributable, exit-bounded treatment.** `MAJ-01`'s objection was to the *treatment*, not the diagnostics: the allow-list held twenty-one **bare** `(rule, file)` pairs, which record that something was tolerated but not who decided, on what grounds, or when the decision stops holding. Every entry now carries a resolvable `RISK-` reference and a written disposition, the step **fails** on a missing, unresolvable or empty one, and both forms of the list are published (`security-allowlist.txt` for the comparison, `security-allowlist-justified.txt` for the reasoning). The two `CA5351` pairs — the legacy MD5 **verification** path the credential migration requires, and a digest helper outside the remediated key handling — have their own record, `RISK-171`, carrying the grounds, the compensating controls and a **measurable exit condition** (`SELECT count(*) … WHERE password ~ '^[0-9a-f]{32}$'` reaching zero retires the acceptance). Deleting the path instead would lock out every user whose hash is not yet upgraded, which the preservation requirement forbids. **One thing is still open and is stated rather than absorbed:** an acceptance is the owner's to ratify, not this remediation's, so `RISK-171` carries the ratification token unrecorded exactly as `RISK-001` does for the `AutoMapper` licence. **Re-measured at this revision:** the armed build completes with **zero errors** and **3,055** warnings, and reports **55** distinct Security-category diagnostics across **five** rules — `CA2100` (20), `CA2326` (20), `CA2328` (9), `CA5351` (5) and `CA5362` (1) — which collapse to **21** `(rule, file)` pairs; the ratchet's set difference against the reviewed-residual allow-list is **empty in both directions: zero unreviewed and zero stale**. So the gate's criterion, *no unreviewed Security-category diagnostic*, is met exactly — and note what that does and does not say: there are 55 diagnostics, not none, and every one of them is an accepted residual with a reference and a reason. See the Gate 1 detail below, `RISK-051` and `RISK-171`. |
...
| --- | --- | --- |
| Minimal Change guideline 9 — **atomic commits per vulnerability class** | **FAIL — NOT COMPLIANT for this milestone; single-class discipline holds from the failure onward** | Seven of thirteen commits in the original remediation carry more than one vulnerability class. Every commit made while closing the code-review findings carries exactly one class — the run of consecutive single-class commits is unbroken since — so the requirement is met *going forward*, which is not the same as the milestone meeting it. **History was not rewritten, and code-review finding `MAJ-04` requires the disposition to be stated rather than left implicit: rewriting published history is not something an automated remediation may authorise itself to do**, so the only options were to rewrite (unauthorised) or to report the milestone as noncompliant. This row is the second. The remediation log records the requirement as *NOT MET* and explains why. **Acknowledgement is not compliance**, and no statement in this document set may cite the acknowledgement as though it were — nor may the forward-looking half be read as retiring the failure. |
...
| **Frozen operation map** — 86 files, 12 CREATE, 74 UPDATE, 0 DELETE | **DEVIATED — not compliant** | Measured against the pre-engagement commit `c8ea6bd4`: **174** changed paths — **25** added, **145** modified, **4** deleted. All 86 authorised paths were changed, and **88** further paths were changed that the map does not authorise, including **4 deletions where the map authorises none**. *(These figures read 171 / 24 / 143 / 85 until runtime finding `F-130` re-measured them. Two of the three deltas pre-date that finding — the revision that closed `F-129` added one modified and one added path — and the third is `F-130`'s own single modified path, `WebVella.Erp.Plugins.SDK/Pages/tools/cogegen.cshtml`. Reproduce with `git diff --name-status c8ea6bd4 HEAD \| awk '{print $1}' \| sort \| uniq -c`.)* Raised by code-review finding `MAJ-04`, which specifically refused the argument that extra changes are compliant because they carry security rationales, and escalated by code-review finding `CR-01`, which refused the further argument that a section inside this document could *amend* the frozen plan. **It cannot, and no longer claims to.** The deviation is DISCLOSED path by path with the finding that prompted each change, and it stays non-compliant: the two authoritative resolutions — reverting the 85 unauthorised paths and 4 deletions, or reissuing the plan — are both owner decisions, and reverting would reintroduce closed Critical and High remediations. See the [disclosure and escalation](#scope-deviation-disclosure-and-the-escalation-it-raises-cr-01-maj-04-maj-10-maj-11) below. |
...
| **Preservation requirement — performance within 10% of baseline** (one pre-declared exception: the login key derivation) | **PARTIAL — one regression FIXED, one measured residual ACCEPTED and owner-owned** | A performance checkpoint measured this build side by side against the pre-engagement commit `c8ea6bd4`, published under the same SDK against an identically seeded database, interleaved so host contention hit both equally. **The pre-declared login exception is verified and honoured**: the work factor is exactly 600,000, a login costs exactly one derivation, no degenerate input reaches a derivation at all (0.03–0.08 of one), and case-variant duplicate accounts cannot amplify it (ratio 1.01×). **Two non-login boundary breaches were reported. One is fixed:** a bearer request consulted the session-revocation store **twice** for the same credential, which is now answered once — `plugin_data` probes per bearer request measured **1.97 → 0.96**, and the read shape that had breached moved from **+13.1% to +1.8%**, with every measured shape now inside the boundary and revocation itself re-verified (a replayed cookie and a replayed bearer token are both still refused on the **first** request after sign-out). **One is not, and is `RISK-184`:** the longest main-thread task per page load remains **+15 ms on the least-contaminated statistic, +18.5 ms on the paired median**, and a third build differing **only** in the suppression of `Content-Security-Policy-Report-Only` sits at **exact parity** with the baseline (paired median +0.0 ms) — so **100% of the residual is that one mandated header**, from 66–68 violation reports per load of which 64 are inline styles injected mostly by vendor code. It is not reducible without weakening the frozen header value, which the Security Headers standard above forbids and which the reporting checkpoint itself instructed against. The checkpoint's other named cause — the CWE-754 upload-refusal control appended to the render-blocking in-head `site.js` — **is fixed**: it now ships as the `defer`-loaded `wwwroot/js/upload-rejection-feedback.js`, `site.js` is byte-identical to the pre-engagement file, and the render-blocking resource set matches the baseline's exactly. Reported partial rather than met: all Core Web Vitals stay in the "good" band, the rendered pages are byte-identical PNGs and no functionality is affected, but a measured 15–18 ms is a measured 15–18 ms. See `RISK-184`–`RISK-187`. |
...
### Two executing cross-site-scripting defects were found after this inventory was frozen, and both are closed

Recorded on this surface because both were **Critical**, both were proven to execute arbitrary script in a
real browser on the application's own origin, and one of them was **introduced by this engagement**. They are
`Q1` and `Q2` of the final frontend verification. Neither is added to the 53-finding Part 1 inventory or to
Parts 2 through 5: that census is asserted by the release gate and cited by count across this document set,
and `RISK-118` established the convention that a discovery made after the inventory was frozen is recorded
where it was found rather than by renumbering. The full write-ups are in
[the remediation log](remediation-log.md); this row is the status.

| Finding | What it was | Status |
| --- | --- | --- |
| `Q1` | **A regression this engagement introduced.** Correcting the screen-message encoder for its JavaScript string context (review finding `L-OPEN-04`) delivered the value's true characters into `toastr`, whose 2.1.3 default `escapeHtml: false` assigns **both** the message and the title arguments as HTML. Attacker-influenceable text — an SMTP peer's own response text, reaching the sink through `email.ServerError` — therefore became DOM that executed with the operator's authenticated session | **CLOSED.** `"escapeHtml": true` on both per-call option objects in `ScreenMessage/Default.cshtml`. Re-verified at runtime against a hostile SMTP peer: the toast holds one text node, `childElementCount` 0, zero `[onerror]` elements, zero images in the container, the probe URL never requested, and no `SyntaxError` — so the two defects the earlier fix closed stay closed |
| `Q2` | **A scope gap in `H-06`.** The stored page label reaches thirteen page views as `ViewData["Title"]` and was concatenated **unencoded** into the document title tag. Because that element is RCDATA, a label carrying its matching end tag exited it and the parser built a real script element inside the head and executed it during parse. Reachable and triggerable by the **lowest-privileged authenticated role**, and visually silent | **CLOSED.** The label is HTML-encoded where the tag is built, in `HeadTopIncludes.cs`. Re-verified at runtime: the 91-character payload renders as text in exactly one title element with a single text node, **zero** inline head scripts against 13 external — identical to both controls — the window flag never created, and the encoding confirmed in the delivered bytes rather than only in the DOM |

**What this changes on the `H-06` row of the standards table above, stated precisely.** The Injection
Prevention standard row still reads *satisfied for every finding it governs*, and that remains true — but
`H-06`'s **scope** is wider than the audit's own enumeration of it. The audit named the navigation, menu,
data-source and widget sinks; the title sink consumed the same field and was not named. The decisive
evidence is worth keeping on this surface because it generalises: on a **single HTTP response** the identical
stored value was inert at the remediated menu sink and executing at the unremediated title sink. A stored
field is only as safe as its least-encoded consumer, and an audit that enumerates sinks rather than
*consumers of a field* can miss one. The corresponding recommendation — audit every consumer of a
stored value rather than every sink an earlier pass listed — is the one methodological lesson of this pass.

**What it does not change.** The report-only content policy observed both defects and blocked neither, which
is `RISK-022` working exactly as documented and is not a new finding. Gate 1's substitute static analysis did
not report either defect: both live in a Razor view and a view component, and neither shape is in the
Security-category rule set — recorded so that a clean analyzer run is not read as evidence that no sink
remains.

### Scope deviation disclosure, and the escalation it raises (`CR-01`, `MAJ-04`, `MAJ-10`, `MAJ-11`)

> **THIS SECTION DOES NOT AMEND THE AGENT ACTION PLAN, AND CANNOT.** An earlier revision was headed
> *"Formal scope and contract amendment"* and opened *"This section is the formal amendment the code
> review required."* Code-review finding `CR-01` rejected that, and the rejection is correct on its own
> terms: **the plan is frozen, and an implementation cannot widen its own authorisation by writing a
> section that says it has.** Labelling a deviation an amendment inside the deliverable it deviates from
> is not governance, it is self-authorisation. The claim is withdrawn, in place, rather than deleted, so
> the correction is visible to a reader who saw the earlier wording.
>
> **What this section is instead:** a disclosure. It records, path by path, what was changed outside the
> frozen map and which finding prompted each change, so the deviation is legible and reviewable. Every
> path listed below remains **UNAUTHORISED by the frozen map**, and the status row above therefore
> reads **DEVIATED — not compliant** and continues to. Nothing here should be read as making it
> compliant.

#### The escalation, stated plainly

`CR-01` offers exactly two authoritative resolutions, and **neither is one an automated remediation may
choose**:

1. **Restore the 85 unauthorised paths and the 4 deletions.** This would revert closed Critical and High
   remediations — the review said so itself — including findings a *later* review raised and this
   engagement then closed, among them an authenticated remote-code-execution path (`CK-01`), a stored
   cross-site-scripting sink at a client-side `innerHTML` assignment (`SR-05`), a privilege-escalation
   route on the `user_role` relation (`CR-01` of the checkpoint review) and the durable session
   revocation and login lockout (`H-OPEN-01`, `H-OPEN-02`). Reintroducing known-exploitable defects to
   satisfy a path count is a decision with security consequences that belongs to the repository owner,
   not to the agent whose own work would be reverted. **It has therefore not been done, and the refusal
   is recorded here rather than presented as a judgement call already settled.**
2. **Obtain a new authoritative plan.** By construction this cannot originate inside the
   implementation — that is the very move `CR-01` forbids. It requires the owner to reissue the plan with
   the additional paths authorised.

So the honest disposition is the third thing, and it is the only one available: **report the deviation,
report that it is not compliant, and escalate the decision unresolved.** The precedent is the
engagement's own, and it is applied deliberately rather than invented here — AAP §0.6.5 requires four
escalations to be *"surfaced rather than absorbed"*, and §0.9.2 states that an automated agent must not
change a product's posture on its own initiative. `RISK-001` already leaves the `AutoMapper` licence
question open for exactly that reason, and mechanically blocks `dotnet pack` rather than deciding it.
This deviation is now carried the same way.

**What the reader must take from this.** The engagement's scope requirement is **NOT MET**. The
milestone is not complete. No sentence in this document set, in `SECURITY.md`, in `LIBRARIES.md`, in
`docs/index.md` or in `catalog-info.yaml` may cite the disclosure below as though disclosure were
compliance — and where an earlier revision of any of them did, it has been corrected. The inventory that
follows exists so the owner can act on a complete and accurate account of what changed, which is the one
thing the implementation *can* contribute to resolving this.
...
| --- | --- | --- | --- |
| Total paths | **86** | **174** | +88 |
| CREATE / added | **12** | **25** | +13 |
| UPDATE / modified | **74** | **145** | +71 |
...
disclosure uses the larger, correct figure.

#### The twelve additions the map does not authorise

**"Authorising finding" below means the finding that PROMPTED the change, not an authorisation.** Only a
reissued plan can authorise a path, which is `CR-01`'s point and is why the column heading is read that
way throughout this disclosure. The map authorises 12 CREATEs and all twelve exist as specified. Twelve
**further** files were created, none of them authorised:

| Added path | Prompting finding (NOT an authorisation) | Why a new file rather than an edit |
| --- | --- | --- |
...
| Deleted path | Prompting finding (NOT an authorisation) |
| --- | --- |
...
#### The seventy-two further modifications, by area

The count is **seventy-two**, derived rather than asserted: **145** paths were modified against the
pre-engagement commit `c8ea6bd4`, the frozen map authorises **74** UPDATE paths, every one of those 74 was
modified, and `comm` over the two sorted sets leaves exactly **71** modified paths the map does not
authorise; the seventy-second entry is `WebVella.Erp.Web/wwwroot/js/upload-rejection-feedback.js`, which
this list has always carried and which is an unauthorised **addition** rather than a modification — stated
here because the arithmetic otherwise looks one out. Reproduce it with
...
where the measured figure is **17**. Both were corrected and the rows summed to 69.

**Then the figure moved twice more, and both moves are recorded rather than absorbed.** It read
*sixty-nine* against **143** modified paths until runtime finding `F-130` re-measured it. At the revision
before that finding the true figures were already **144** modified and **70** unauthorised, because the
revision closing `F-129` modified two web-framework files this list did not name —
`WebVella.Erp.Web/Components/HeadTopIncludes/HeadTopIncludes.cs`, which is where `F-129` was fixed, and
`WebVella.Erp.Web/Components/HeadBottomIncludes/HeadBottomIncludes.cs`. Both are now named, which is why the
web-framework row reads **19** rather than 17. `F-130` itself added exactly one:
`WebVella.Erp.Plugins.SDK/Pages/tools/cogegen.cshtml`, taking the SDK row to **11** and the modified count
to **145**. A scope-deviation count that moves without explanation is indistinguishable from one that was
adjusted to fit, so each move names the finding and the paths behind it.

| Area | Paths | Prompting findings (NOT authorisations) |
| --- | --- | --- |
| Blazor WebAssembly client — **excluded by name in AAP §0.3.2** | 12 | `SR-01`, `SR-02`, `SR-03`, `SR-06`, `SR-08` (commit `cdb1edb9`), plus the sign-out revocation in `80042d8c`. The exclusion was written on the premise that the client's only security-relevant behaviour was outbound HTTP and that server-side request forgery did not apply — which remains true, but the seam review found credential-handling defects in the client's own auth layer that no server-side change can reach. Two further pre-existing defects found there were left alone at the time and recorded as `RISK-119`; **both have since been fixed** - on the client and, for the scheme-casing half, on the host selectors as well - and that record is now closed. |
| Project plugin — components, controllers, services | 13 | `SR-05`, `SR-07`, `SR-09`–`SR-16`, and this review's `MED-01` |
| Web framework — remaining `.cs`, `.cshtml` and `.js` | 19 | `H-06` (`PcHtmlBlock`, `PcPageHeader`, `ScreenMessage.Default.cshtml`), `H-13` (`ApiControllerBase`), `H-02`/`H-03` (`JwtMiddleware`), `M-12` (`WebSecurityUtil`), `HIGH-01`, and `SR-11`/`SR-13` with review findings `N18`/`N20` (`wwwroot/js/upload-rejection-feedback.js`, `WvFieldUserFileMultiple/inline-edit.js`). The row previously read *remaining `.cs`* at **16**; the extension list is corrected because the seventeen paths are **14** `.cs`, **1** `.cshtml` and **2** `.js`, and a row headed `.cs` cannot account for three of its own members |
...
| SDK plugin | 11 | `H-06` reflected and stored sinks beyond the three named views, `H-09`/`H-10` (`CodeGenService`), `M-09` (`AdminController`), and `F-130` (`Pages/tools/cogegen.cshtml`) |
...
##### The seventy-two paths, enumerated (`N38`)
...
seventy-two is therefore named here, in the same areas and summing to the same counts. Each remains
...
**Web framework — remaining `.cs`, `.cshtml` and `.js` (19)**

- `WebVella.Erp.Web/Components/HeadBottomIncludes/HeadBottomIncludes.cs`
- `WebVella.Erp.Web/Components/HeadTopIncludes/HeadTopIncludes.cs`
...
- `WebVella.Erp.Web/wwwroot/js/upload-rejection-feedback.js`
...
- `WebVella.Erp.Plugins.SDK/Pages/tools/cogegen.cshtml`
...
Every deviation strengthens the gate, so none is **aligned down** — the review directed that the stronger
posture be preserved during reconciliation, and it is. That is a decision about *which way* to reconcile,
and it is worth being exact about what it is not: it does not make the divergence authorised. A gate that
is stronger than the frozen text still differs from the frozen text, so each row below is a disclosed
deviation escalated with the rest, per `CR-01`.
...
| Distinct analyzer diagnostics | **3,028** unique `(file, line, column, rule)` `CA` pairs, spanning **50** distinct rules and **641** distinct `(file, rule)` pairs. The raw log carries 6,056 `CA` warning lines because a multi-project build restates each one; de-duplication is therefore mandatory, and MSBuild's own summary total of 3,055 counts every category, not just `CA` | de-duplicate the build log's `CA` warning lines on the repo-relative path |
...
| Taint family `CA3001`–`CA3012` | **0** diagnostics, over **19 of 19** compilations. The family is armed for eighteen projects in the solution build and withheld from the **build** of `WebVella.Erp.Web` alone on a measured termination bound (`RISK-051`, `RISK-138`); under review finding `MAJ-01` that nineteenth compilation is scanned by its own workflow step with the family armed and the interprocedural chain bounded to one hop — **about 220 s, exit 0, zero diagnostics**, behind a capability self-test that requires `CA3001` and `CA3003` against deliberate flows. The residual is analysis *depth* for that one project, not coverage | same filter for the eighteen; `taint-scan-web.txt` for the nineteenth; the exclusion is asserted in both directions by the workflow |
...
| Workflow shape | **25** steps — **3** `uses:` and **22** `run:`, publishing **24** evidence paths | load the YAML and count `steps` entries carrying each key; count the `path:` lines on the upload step |
| Gate 5 declared rows | **50** — `A01`–`A18` plus `M01`–`M32`; **31** of the 32 manual rows are REQUIRED and one, `M18`, is ADVISORY. All 32 hold a committed attestation, so the matrix reports `deferred=0` | `grep -n 'expected_rows=' .github/workflows/security-scan.yml`; `grep -c '^M' manual-verification-results.txt` |
...
| Remediation log sections | **46** `## ` sections. This figure has now drifted five times — 24, then 39, then 40, then 44, now 46 — so read it as a measurement of one revision and re-derive it rather than citing it. Two of the two-step move are accounted for separately and honestly: the final frontend verification added **one** section, and the previously stated 44 already disagreed with the tree, which the script returned as **45** before that section existed — an off-by-one that pre-dated this pass and is corrected here rather than carried forward | `grep -c '^## ' docs/security/remediation-log.md` |
...
then 16/3/13, then 17/3/14, then 23/3/20, now **25/3/22**), the Gate 5 row total (from 32, then 33, then
41, then 45, then 48, to **50**),
...
the solution build reports **3,055** warnings at this commit, none escalated — and promoting that to errors
would demand precisely the mass refactor the engagement's modification boundaries forbid. That figure read
**3,093** before the frontend and API seam remediation, **3,062** after it, and has only ever moved
**downwards**; the accounting, including the seven-warning reduction that took 3,062 to 3,055, is in the
state-of-the-remediation table below.
...
**Gate 5 — manual verification of all Critical and High fixes.** *The manual half is now complete: every
declared manual row has been executed against a running deployment and holds a committed attestation.* Two
earlier revisions of this section were wrong in opposite directions. The first read "*Satisfied*", which the
evidence did not support and which a later review correctly rejected: the checklist in the
[remediation log](remediation-log.md) is a **procedure** per vulnerability class, and a procedure is not an
execution. The second counted the unexecuted rows honestly as `DEFERRED`. This revision records what
changed — the rows were executed. Thirty-two of the scenarios need a live PostgreSQL instance, a browser or
an SMTP server, none of which the automated gate has; all thirty-two have since been executed by hand,
against disposable databases and disposable hosts, and attested.

What the gate does, and what it now measures:

- The verification matrix carries **50 rows: 18 derived from an evidence artifact the workflow itself
  produced (`A01`–`A18`), and 32 manual (`M01`–`M32`).** Running the matrix logic against this tree with the
  ...
  revision of this bullet read 48 rows with `proven=48 failed=0`; the row count and the verdict have both
  moved, and the earlier verdict rested on a scenario that had been rewritten to accept the report-only header
  name — which `MAJ-03` refused.)*
...
characterised — `RISK-170` inventories all 109 raw-output sinks with their writers and authorization
...
| Area | State |
| --- | --- |
| Solution restore and build | `dotnet restore WebVella.ERP3.sln` exit 0 with zero `NU19xx`; `dotnet build WebVella.ERP3.sln -c Debug` exit 0, **0 errors**, **3,055** analyzer warnings, none escalated. **Re-measured with `--no-incremental` at this commit**, which matters: an *incremental* solution build reports a single warning because nothing recompiles, so it cannot be used for regression comparison. The history of this figure, since every movement is accounted for rather than replaced: it read 3,096 while a repository-root `.globalconfig` armed additional rules; 3,094 after that file was removed and the final regression pass replaced two `throw new Exception` statements in `WebVella.Erp/Database/DbFileRepository.cs` with `FileNotFoundException`; **3,093** after the page-header encoding reconciliation; **3,062** after the frontend and API seam remediation, which is the figure the [remediation log](remediation-log.md) records for the tree that pass published; and **3,055** at this commit, after three later Medium remediations removed seven more. **This row has now stood stale twice, and both corrections are recorded rather than overwritten: it read 3,094 until the seam remediation, one revision behind the remediation log's 3,093 — which review finding `SR-13` exists to catch — and it then read 3,062 for one revision after the tree had reached 3,055, which is QA finding `DOC-02`'s sibling `DOC-01`.** The last seven are attributed rather than absorbed, per this row's own standard: `CA1854` ×2 in `WebVella.Erp.Web/Services/CodeEvalService.cs` (commit `087ad593`, `M-OPEN-04`), `CA2201` ×4 in `WebVella.Erp.Plugins.Mail/Api/EmailServiceManager.cs` (commit `53024964`, `M-OPEN-06`) and `CA2201` ×1 in `WebVella.Erp.Web/Services/MailService.cs` (commit `4e41a093`, `M-OPEN-03`) — measured by building the 3,062 tree and this one and diffing the unique `(rule, file)` sets, so the accounting is exact and not inferred. Every step of the movement is a **reduction**, and the two contributors are named: retiring four files under `SR-04`, and replacing five `throw new Exception` statements with typed exceptions under `SR-08`. **The warning-code multiset was diffed before and after that remediation and is identical in composition — 53 codes — so zero new warning codes were introduced; re-diffed against the 3,062 tree it is still exactly those 53 codes, so the seven-warning reduction removed diagnostics without retiring a code or adding one.** The composition at this commit, which reconciles the total exactly: **3,028** `CA`, **21** `ASPDEPR`, **5** `CS` and **one codeless** warning (`libman.json does not exist`, emitted by the client-library restore target and carrying no diagnostic identifier) — 3,054 coded plus 1 codeless, which is why a code-only census of this build reports 3,054 rather than 3,055. Reducing those coded warnings to unique `(file, line, column, rule)` sites gives **3,047** — 3,028 `CA` plus 19 non-`CA` — which is the figure the gate ratchets against |
...
contains **109 invocation sites across 61 views** — 111 until runtime finding `F-130` closed two more in
`WebVella.Erp.Plugins.SDK/Pages/tools/cogegen.cshtml` — re-measured under code-review finding `MED-03` with Razor (`@* *@`) and HTML (`<!-- -->`) comment blocks stripped first, so the figure counts active code only. *(An earlier revision of this sentence read 114 across 62. That figure was accurate for the tree at commits `88136e3b` through `0f7419ae` and went stale at commit `859356e4` (`L-OPEN-04`), which replaced the three raw wrappers in `WebVella.Erp.Web/Pages/ckeditor/ImageFinder.cshtml` with script-context serialisation — one view, three calls, and the whole of the difference. Stripping comment blocks does not change today's number: the plain `grep -roE 'Html\.Raw\(' --include=*.cshtml` also returns 111 across 61, because no surviving invocation sits inside a comment. What the looser pattern `Html\.Raw` without its parenthesis returns is **118** across **67**, the extra matches being prose, XML-documentation remarks and* do not reintroduce *comments. `RISK-170` in the risk register is canonical for this census and reproduces it with the command that produces it.)* Within the original census,
...
`Html.Raw(` in `.cs` files is **0**; the single `.cs` occurrence referred to elsewhere **was**
`helper.Raw(input)` inside `WebVella.Erp.Web/Utils/HtmlHelperExtension.cs`, which is finding `M-18`.
That call no longer exists at this revision — the helper is `WvJsonRaw` at `:L29` and its body emits
`input.Replace("<", ScriptBreakoutEscape)` at `:L34` — so `helper.Raw(` now returns **0** matches too,
and the `.cs` census is zero by construction rather than by exclusion. (An earlier revision cited
`:L11` in the pre-remediation 14-line file; that line is a comment in the 37-line file shipped now.)
In the SDK data-source listing specifically, `:L25` is the excluded `record["action"]` site, while
`:L38`, `:L42` and `:L47` already use auto-encoded plain expressions — `@record["name"]`,
`@record["type_description"]` and `@record["target"]`. (An earlier revision cited `:L39` and `:L43`,
which are the `</wv-grid-column>` closing tags one line below each expression.)
...
*at the rename*, and it has since moved twice, each time for a stated reason rather than by drift.
[Part 4](#part-4-the-frontend-and-api-seam-review) added sixteen records in the previously unused `SR-`
namespace, taking the tree to 115 finding records and 115 distinct identifiers, and the workflow's
`expected_minimum_records` was raised from 96 to 115 in the same change the records were added — which is
what that step's own instruction requires. [Part 5](#part-5-the-checkpoint-code-review-of-the-remediation-itself) then
added the checkpoint review's twenty-three `CK-` records, so **the tree now measures 138 finding records
and 138 distinct identifiers**, and `expected_minimum_records` was raised from 115 to 138 in that same
change. *(Code-review finding `MED-04` found this paragraph still stopping at 115; the earlier totals are
retained above as explicitly historical measurements rather than deleted, and the current figure is the one
the workflow asserts on every run — it printed `138 finding records … carry 138 distinct identifiers, with
the Part 1 inventory at 53` when the step was last executed against this tree.)* The
...
| **LOCATION** | `WebVella.Erp/WebVella.Erp.csproj` (the package pin: `:L47` before the remediation, `:L72` at this commit after the escalation comments were added above it) and `WebVella.Erp/Api/Models/AutoMapper/ErpAutoMapper.cs` (the single mapping-configuration construction site the upgrade required to change: `:L15` before, `:L18` at this commit — the file is 21 lines, so the `:L23` an earlier revision cited was past its end). A third location joins them after `CR2-F-04`: `Directory.Build.props`, which carries the licence-governance gate that keeps the unratified claim unpublishable |
...
| Field | Value |
| --- | --- |
| **FINDING** | Every shipped `Config.json` carries a live PostgreSQL connection string — database user name and password included — and a live encryption key; two of the eight additionally carry the token signing key (finding `H-04`). *(This field read "and one also carries the mail account password" until code-review finding `MED-05`. That was false and had already been corrected two cells below, in DESCRIPTION and EVIDENCE: `EmailSMTPPassword` is the empty string in **all eight** files. The correction is now stated in the FINDING field itself, because a reader who takes only the first line of a record must not be left with a claim the record's own evidence contradicts.)* |
...
| Field | Value |
| --- | --- |
| **FINDING** | The raw-output helper is used at 128 sites across 69 Razor views — **the census as taken at the audit baseline, which is the count this record is written against throughout.** It is not the current count: the remediation deleted raw wrappers at the confirmed text sinks, so the tree as delivered carries **109 invocation sites across 61 views** — corrected in this field under code-review finding `MED-03`, which found it still quoting a superseded figure after the narrative and the summary matrix had been corrected, and corrected again under runtime finding `F-130`, which closed two further sinks in `WebVella.Erp.Plugins.SDK/Pages/tools/cogegen.cshtml` and moved the figure from 111 to 109. Both numbers are stated wherever current state is discussed, and neither substitutes for the other — **128/69 is the original audit census**, **109/61 is the current census**, and `RISK-170` is canonical for it. The current figure is reproducible rather than asserted: `grep -roE 'Html\.Raw\(' --include=*.cshtml . \| wc -l` returns 109 and `grep -rlE 'Html\.Raw\(' --include=*.cshtml . \| wc -l` returns 61; stripping Razor (`@* *@`) and HTML (`<!-- -->`) comment blocks before counting returns the same **109 across 61**, because no surviving invocation sits inside a comment. ~~114 invocation sites across 62 views~~ was this field's earlier claim, and it was accurate for the tree at commits `88136e3b` through `0f7419ae`; it went stale at commit `859356e4` (`L-OPEN-04`), which replaced the three raw wrappers in `WebVella.Erp.Web/Pages/ckeditor/ImageFinder.cshtml` with script-context serialisation — that single view is the entire three-call, one-view difference, and the delta is deleted code rather than commented-out code. Counting *invocation* sites rather than every textual mention still matters: the looser pattern `Html\.Raw` without its opening parenthesis matches **117** occurrences across **67** views, because it also matches the places the helper is merely named in prose, in an XML-documentation remark or in a *do not reintroduce* comment, which is how a census can drift without any code changing. That loose figure is unchanged by `F-130`, which is itself instructive: it removed two invocations and added two prose mentions in the same file, so only the sink-only count moved. (This field read **118** until `F-130` re-measured the loose pattern at 117 on both the delivered tree and the revision before it, making the 118 an off-by-one rather than a stale reading.) Three of them emit an unencoded, caller-supplied return URL into an `href` attribute; a further set render database text without encoding, including navigation and menu content that appears on every page of every host. |
...
The heading read *three* until runtime testing of the delivered tree raised `F-129`, the fourth, which is set
out fourth because it is the one that indicts the census **method** rather than a single classification — and
*four* until a later runtime pass raised `F-130`, the fifth, which is set out last because it is the one the
census could have caught and did not.
...
**`F-129` — the `<title>` element, and why the census method could not see it.**
`WebVella.Erp.Web/Components/HeadTopIncludes/HeadTopIncludes.cs:L29` composed the document title by
concatenation — `"<title>" + pageModel.PageContext.ViewData["Title"] + "</title>"` — and
`HeadTopIncludes/Default.cshtml:L12` emitted the finished string through `@Html.Raw(metaTitle)`. Twelve page
models assign `ViewData["Title"]` from `ErpRequestContext.Page.Label`, administrator-editable database text,
and one of them is `Pages/Site.cshtml:L8`, which serves the `/s/{name}` route. So a stored label reached the
browser as markup on every one of those twelve routes.

**Why `<title>` is not self-protecting.** The element is RCDATA, so a bare `<script>` inside it genuinely is
inert — which is the intuition that makes this sink easy to wave past. But a literal `</title>` **terminates
the element** and returns the tokenizer to the normal markup state, after which the remainder of the stored
value is parsed as ordinary HTML. A label of `</title><script>alert(1)</script>` therefore executed on the
application's own authenticated origin. Runtime testing measured it: `</title>` occurred **twice** against a
single `<title>`, the document carried **16** script elements against 15 on a control page, and the payload's
own side effect — a global set *before* it called `alert` — was observed present.

**Why the census missed it, which is the part worth carrying forward.** The classification of the 128
raw-output sites was performed by reading `.cshtml` files and judging each `Html.Raw` **argument**. This
sink's Razor call *was* in that census; what was misjudged is its argument. `metaTitle` looks like
server-composed markup, and it is — but it is markup **assembled in C#** with a database value interpolated
into it, which is exactly the class the census excluded as *"server-generated markup interpolating only
identifiers"*. A method that inspects the emission site cannot see what a different file put into the
variable. `RISK-170` carries the corrected measure; the durable lesson is that a raw-output census must
classify each argument's **writer**, not its call site.

**Closed** by encoding the value at the single point of composition rather than by touching the emission:
`HtmlEncoder.Default.Encode(...)` is applied to the title text in `HeadTopIncludes.cs` before it is
concatenated, with a `?? ""` guard that reproduces the previous null rendering exactly. One writer and one
reader made this the smallest available change, and it covers all twelve routes at once. The encoder is the
one Razor's own automatic encoding uses and the one `BaseErpPageModel.EncodeMenuText` already applies to the
menu sinks, so the two halves of the same value are now protected consistently — a point the finding made
sharply, since the identical label was already inert in the site-menu anchor on the **same HTTP response**
while it executed in the title.

**Verified in a browser, twice, on the hostile fixture.** `</title>` count fell from 2 to **1** and the
script count from 16 to **15**; `document.querySelector('title').childElementCount` is **0** with a single
text node; no script element in the document carries the payload; and the payload's global is `undefined`
with `hasOwnProperty` false — measured both with an `alert` tripwire installed and again on a pristine load
with the genuine native `alert` in place, so no dialog was masked. Behaviour is preserved, and the sharpest
proof of that is the Unicode fixture: `HtmlEncoder.Default` emits non-ASCII as numeric character references,
so the wire carries `&#x1F680;` rather than the raw bytes, yet because RCDATA decodes character references
`document.title` holds the genuine `测试`, `مهمة` and `🚀` characters with **zero** U+FFFD and no literal
entity text. Control titles (`Home`, `Datasources`) are byte-identical to before.

**`F-130` — the SDK code-generation preview, the one sink the census listed and still misjudged.**
`WebVella.Erp.Plugins.SDK/Pages/tools/cogegen.cshtml` renders the Code-generation "Changes" grid, and three of
its columns went through the raw helper: `record.Element` at `:L85`, `record.Name` at `:L103` and each change
description at `:L112`. Unlike `F-129`, all three **were** in the raw-output census — as `RISK-170` class K,
dispositioned *"left as-is — server-composed preview markup"*. Two of the three were live.
`MetaChangeModel.Name` is assigned at 40 sites in `CodeGenService.cs` and every one is free-form database text
— a data source, role, application, page, sitemap area, group, node or entity name or label. Each change
description is assigned at 113 sites and every one interpolates the same kind of value into this tool's own
label markup, for example
...
`record.Element` is the exception: 40 assignments, all hard-coded literals, so it was latent rather than live. Post-remediation the same three columns are at `:L98` (`@record.Element`), `:L116` (`@record.Name`) and `:L125` (`@Html.Raw(CodeGenService.RenderChangeDescription(change))`), with the renderer at `WebVella.Erp.Plugins.SDK/Services/CodeGenService.cs:L9387`, its allow-list at `:L9330` and its token matcher at `:L9433`; the shift of thirteen lines is the Razor comment recording the threat, which Minimal Change guideline 6 requires.

**Measured before the fix, against a running host.** A data source named `qa_ds_<script>alert('ds-xss')</script>`
and a role named `qa_role_<script>alert('role-xss')</script>` — created through the ordinary screens, which
accept them without complaint — produced **three native dialogs** (`role-xss`, `role-xss`, `ds-xss`), **three
live `<script>` elements** inside `#wv-tab1`, and **three unencoded `<script>` sequences with zero encoded
forms** in the raw server response. The visible text of the Name column showed only `qa_role_` and `qa_ds_`,
because the remainder had been parsed as markup rather than displayed. The same values were correctly encoded
in the *adjacent Code pane of the very same response*, which localised the defect to these three sinks rather
than to a general encoding failure. The Content-Security-Policy is report-only, so nothing intervened.

**Why the census listed it and still got it wrong.** This is `F-129`'s lesson repeated at a row that had
already been written down. Class K's Writer column named the page's own builder and its Trust boundary column
named *administrator*, because the values arrive at the view already assembled as markup. But the boundary
that matters is not who may **view** the preview; it is who may **write** the name of a compared object, and
that is any ordinary schema or record write. Applying class B's own conclusion — classify by writer, not by
call site — to class K would have re-opened it without a runtime finding.

**Closed, and each of the three sinks by the treatment its value warrants.** `record.Element` and
`record.Name` are now plain Razor expressions, so Razor's automatic encoding applies and the two raw calls are
gone; that is the whole of the census move from 111 to **109**. The change descriptions keep one raw call, fed
by a new `CodeGenService.RenderChangeDescription`, which walks the composed string once and copies a fragment
only on an **exact ordinal match** against the four this service is permitted to compose —
...
element or handler therefore render as inert text: the renderer fails closed. Deleting this third wrapper as
well would have encoded the tool's own label markup and printed all 113 descriptions as visible HTML source,
which is the regression this report already records for the navigation and widget views; `HtmlSanitizer` was
the other candidate and would have stripped the `class` attribute that review finding `N23` deliberately
removed from its allow-list, so the colour labels would have lost their styling. Encoding at the 113
composition sites — this report's usual doctrine — was rejected as 160 interpolation holes that protect none
of the sites written next.

**Verified after the fix on the same fixtures and the same route:** **0** dialogs, **0** payload `<script>`
elements in `#wv-tab1`, **0** `<script>alert` sequences anywhere in the 50,257-byte response, and the payload
present as inert literal text in all three cells (`qa_role_&lt;script&gt;alert(&#x27;role-xss&#x27;)&lt;/script&gt;`).
Preservation was measured in the same run rather than assumed: the two diff rows, the amber `updated` and
green `created` labels, the green `name` label and both red before/after values still render as markup at
computed `rgb(76, 175, 80)` and `rgb(244, 67, 54)`, the row count is unchanged at three, the Code pane still
emits its 31 lines of generated C#, and there were zero console errors and zero responses at status 400 or
above. Seventeen unit-level checks of the renderer were run against the built assembly, covering the null and
empty inputs, an uppercase and a double-quoted span variant, a token bearing an extra attribute, a truncated
token at end of string, the `<>` separator this service emits between relation identifiers, and a payload
nested inside a permitted token. The residual — a stored value that reproduces one of the four tokens byte for
byte renders as that span, one fixed class and no other attribute, so cosmetically only — is recorded in
`RISK-170`, which `CodeGenService.cs` cites at the renderer.
...
| **EVIDENCE** | The concatenations are present verbatim. **This cell previously recorded zero `CA2100` and zero `CA23xx` diagnostics and read that silence as corroboration that values are parameterised. Both halves of that were wrong and are corrected here.** The silence was an artefact of the analysis level: neither rule is enabled by `AnalysisLevel=latest-recommended`, so it could not have corroborated anything. ~~**A third correction is now needed, and it returns this cell to its first position.** `AnalysisLevelSecurity=latest-all` — under which `CA2100` reported at 20 sites and `CA2326`/`CA2328` at 20 and 9 — has been **removed** under review finding `GATE-03`, together with the `.globalconfig` that tuned it. Those rules do not execute again, so their silence is once more the silence of a rule that is not running and carries no signal in either direction. The nineteen individually justified allow-list entries that covered those sites are no longer adjudicated by any automated gate; they are inventoried by hand in `risk-register.md` under `RISK-052`, which is the honest cost of the frozen gate shape.~~ **That third correction is itself withdrawn under review finding `MAJ-07`, which established that it described a tree that is not the shipped one. Only the `.globalconfig` was removed under `GATE-03`; `AnalysisLevelSecurity=latest-all` ships in `Directory.Build.props` and Gate 1 fails if it drifts.** Measured on this tree: `CA2100` reports **20** diagnostics across 8 files and `CA2326`/`CA2328` report **20** and **9** across 6 and 4 — so the rules do execute, and their diagnostics are a signal rather than a silence. Read correctly, that signal says what this finding says: `CA2100` fires at exactly the sites where an identifier is interpolated, which is the residual PostgreSQL forces, and nowhere a *value* is concatenated. All nineteen `(rule, file)` entries are back in the Gate 1 allow-list — 21 entries in total with `CA5351`'s two — each carrying an attributable written disposition and a resolvable `RISK-nnn` reference, so they are adjudicated by an automated gate that fails on a diagnostic at any new pair, with their engineering justifications preserved in `risk-register.md` under `RISK-052`. The parameterisation claim nevertheless rests where it always actually rested: on direct reading of the data layer, where every *value* is bound through `NpgsqlParameter` and only identifiers are interpolated — `CA2100` cannot distinguish the two, which is why it fires at all 8 files and why each is a written disposition rather than a defect. The `CA2327`-reports-zero corroboration is **reinstated** on the same evidence — see the deserialisation finding below. |
...
| **EVIDENCE** | The permissive type-handling settings were present at all **fourteen** sites the audit enumerated — which is the audit's count, not the attachment count: the binder is attached at **20** sites, the fourteen plus six write-side counterparts found later, and the basis of both figures is stated in the `H-10` record in Part 1. Two things are now recorded that the original evidence could not state. **First, the site count is 20, not 14, and the discrepancy is a counting artefact rather than a missed exposure**: a repository-wide search for `TypeNameHandling` returns more hits than there are settings, because several are mentions inside comments. Restricted to lines that actually *assign* the property, `git grep -n 'TypeNameHandling *=' -- '*.cs'` returns **20**, of which **19 attach `ErpSerializationBinder.Instance` on the same line**; the twentieth, `WebVella.Erp/Database/DbEntityRepository.cs:246`, attaches it on the next line of the same object initialiser. **Second, a claim made here is withdrawn.** This cell previously argued that the closure was *machine-checked* by an analyzer that was armed and silent: `CA2327` fires precisely when `TypeNameHandling` is not `None` *and* no `SerializationBinder` is set, it raised **zero** diagnostics across all 19 projects, and a throwaway probe carrying an identical initialiser with the binder omitted *did* raise it alongside `CA2326`. The probe result stands and the reasoning was sound **while the rule was enabled**. **And the withdrawal is itself withdrawn, because its premise was false.** It asserted that `CA2327` no longer executes on the ground that review finding `GATE-03` had removed `AnalysisLevelSecurity=latest-all` along with the `.globalconfig` that accompanied it. `GATE-03` removed the `.globalconfig` and **narrowed the taint exclusion**; it did **not** remove `AnalysisLevelSecurity`, which is set to `latest-all` in `Directory.Build.props` and which the workflow additionally asserts is loaded into the compilation on every run. Review finding `MAJ-07` reported this passage as invalidating gate interpretation, and it did: **the rule does execute, and its zero is therefore evidence in the evidentiary sense after all.** So the machine check stands — re-measured at this revision, `CA2327` raises **zero** diagnostics across all 19 projects while `CA2326` raises 20 and `CA2328` raises 9, which is exactly the signature of type-name handling present *with* a `SerializationBinder` attached, and the throwaway probe carrying the identical initialiser with the binder omitted *does* raise `CA2327` alongside `CA2326`. The enumeration corroborates it rather than substituting for it: **20** assigning lines, 19 attaching `ErpSerializationBinder.Instance` on the same line and the twentieth on the next line of the same object initialiser, verified by reading each site. Both retractions are kept visible rather than deleted, because a record that quietly reverses itself twice is worse than one that shows its working. Carried in `risk-register.md` as `RISK-051`. |
...
| **LOCATION** | **Eight** deployable host pipelines, not seven: the seven `WebVella.Erp.Site*/Startup.cs` files plus `WebVella.Erp.WebAssembly/Server/Program.cs`. Only that eighth host emitted anything, and that was transport security alone, at `Program.cs:L18` and `:L21` of the pre-audit revision. The middleware introduced to close it is `WebVella.Erp.Web/Middleware/SecurityHeadersMiddleware.cs`, registered at `WebVella.Erp.Web/ErpMvcExtensions.cs:L98` for the seven Razor hosts and linked by source into the eighth, where it is registered at `WebVella.Erp.WebAssembly/Server/Program.cs:L38` alongside the `AddHsts` pin at `:L23`. The eighth host was left uncovered for one revision and was reopened by QA finding `P6-04`; see the REMEDIATION field. |
...
| **REMEDIATION** | A middleware emitting all seven mandated headers with the exact mandated values. **The content-security policy ships in report-only mode**, with the enforced value configurable: four components in the platform deliberately emit inline script or author-supplied markup, so enforcing the policy immediately would break working features and violate the requirement that existing functionality be preserved. The value is never weakened — only its delivery mode is staged, and the report-then-enforce path is documented. **The overwrite behaviour is stated precisely here because an earlier revision of this row described it wrongly, claiming the middleware "does not overwrite a header a host has already set, so a host with a stricter policy keeps it" — that is retracted.** The middleware runs `AttachSecurityHeaders` twice, and the two passes behave differently on purpose. The **eager** pass runs before the pipeline continues and assigns every header **unconditionally**, so a value already present when this middleware executes — which, given its position, means one written by a middleware ordered ahead of it — **is replaced**. A host that wants a stricter policy therefore cannot get one by setting a header upstream; the policy text is a compile-time constant and the only bindable setting is the delivery mode, which is deliberate, because a configurable policy string is a configurable way to weaken it. The **response-start** pass then re-attaches only headers that are *missing*, which is what makes the set survive a downstream handler that resets the response, and is why it is a no-op on an ordinary response where the eager pass already wrote everything. Exactly one of the two policy header names is ever emitted, and the re-attach pass tests for **both** names before writing either, so a response can never carry two policy headers. **The middleware is now registered and ordered in all eight deployable host pipelines** - two earlier revisions of this row were wrong about its reach, and both corrections are recorded rather than overwritten. The first said it "has no callers yet"; that was retracted when the seven site hosts adopted it. The second said "all seven pipelines", and that is retracted now, under QA finding `P6-04`: seven was the count of *Razor site* hosts, but this repository publishes an **eighth** deployable ASP.NET Core host — `WebVella.Erp.WebAssembly/Server` — and that host emitted **0 of 7** — measured at the wire on its document, a framework `.js`, the `.wasm`, `appsettings.json`, a `404` and the plaintext redirect, against a published Release build of the unmodified tree run side by side with the fixed one. One nuance in that zero is worth stating precisely rather than leaving as a round number, because it changes what the finding is: the unmodified host *did* call `app.UseHsts()`, and the zero was read at `Host: localhost`, which `HstsOptions.ExcludedHosts` excludes by default. Re-measured at a hostname the framework does **not** exclude, the unmodified host emits **1 of 7**, and that one is non-compliant — `strict-transport-security: max-age=2592000`, thirty days with no `includeSubDomains`. So the accurate statement is that the eighth host emitted none of the six non-transport headers under any hostname, and emitted the transport header only at a real hostname and only at a value the mandated set does not permit. The cause was structural rather than an ordering oversight: that project references only the WebAssembly Client and Shared projects, never `WebVella.Erp.Web`, so the middleware was not in its compilation at all and no amount of pipeline re-ordering could have reached it. `SecurityHeadersMiddleware.cs` is now linked into that project by source (`<Compile Include ... Link="Middleware/SecurityHeadersMiddleware.cs" />`) rather than pulled in by a project reference, because a reference to `WebVella.Erp.Web` would drag Npgsql, CS-Script, AutoMapper and MailKit into a host that is otherwise a static-asset shell; and it is linked rather than copied so the mandated values keep a single definition. `app.UseSecurityHeaders()` is its **first** middleware, outermost, ahead of `UseHttpsRedirection`, `UseBlazorFrameworkFiles` and `UseStaticFiles` — the last two of which terminate the pipeline for a matched asset, which is exactly why anything registered after them shipped bare. That host also required `services.AddHsts(...)` to be pinned to the mandated one-year, subdomain-inclusive value, for the reason set out in the middleware's own comment; the pin is **load-bearing and was measured on the real hosts, not assumed** — probed at a hostname outside `HstsOptions.ExcludedHosts` so the framework's own writer is active, the unmodified host answers `max-age=2592000` and the pinned host answers `max-age=31536000; includeSubDomains`, in both cases as exactly **one** header, so the two writers agree rather than duplicating. Independently reproduced in a minimal two-writer probe toggled by an environment variable, which yields the same pair of values. It sits ahead of response compression and ahead of both static-file middlewares in every one of the eight hosts, and all seven headers were verified **on the wire** against published hosts on a dynamic response, on static assets, on a `404`, on a `405` and on the plaintext `307`. The delivery switch binds from configuration — one key, `SecurityHeaders:ContentSecurityPolicyReportOnly` (environment variable `SecurityHeaders__ContentSecurityPolicyReportOnly`), whose two failure directions differ deliberately and were also previously misdescribed: an **absent or blank** value keeps the compiled report-only default, which is the mandated shipping posture, but a value that is **present and unparseable aborts startup** rather than falling back. It does not "fail safe to report-only" and must not, because either guess would be wrong — assuming report-only would leave the policy unenforced while the operator believed otherwise, and assuming enforcing would block the four components that deliberately emit inline script. The abort names the key and the two accepted values and never echoes the supplied value, so a secret pasted into the wrong variable cannot reach a log; the abort itself surfaces as an unhandled exception, which is recorded as `RISK-124` — so advancing the rollout to enforcement requires no code change. It is the **only** member of the options type bound from configuration; the policy text itself is a `public const`, so no configuration source can weaken it. There is **no** violation-report endpoint: the collector and its `report-uri` directive were removed in full, which is why the emitted value stays byte-identical to the mandated policy and why `Invoke` has exactly one path through it — verified as zero `return` statements and exactly one `await next(context)`. |
...
| **LOCATION** | The defect is inside `WebVella.TagHelpers` and **cannot be edited** — vendor code, version updates only. The remediation vector is `WebVella.Erp.Web/wwwroot/js/upload-rejection-feedback.js`, which every page emits after jQuery, `defer`-loaded so it is guaranteed to run after every synchronous script (see `RISK-184`). |
...
| **LOCATION** | `WebVella.Erp.Web/wwwroot/js/upload-rejection-feedback.js`. |
...
The twenty-three records follow in identifier order rather than in severity bands, because this review's
subject is a single pass over the remediation and its own numbering is the order a reader holding the review
will follow. Severity is stated in each record's `SEVERITY` field. *(This band heading was added under
code-review finding `LOW-02`: Parts 1 through 4 each place a `###` band heading between the part heading and
their `####` records, and Part 5 was the only one that did not, which is what produced the sole
heading-increment diagnostic in this document. No record heading, identifier or field was changed.)*
...
| **EVIDENCE** | Measured per project with the suppression overridden: **eighteen of nineteen complete in 0 to 6 seconds each with zero `CA3001`–`CA3012` diagnostics**, and `WebVella.Erp.Web` alone is killed at a 600-second bound. Summed baseline for the eighteen was 43 seconds against 45 seconds armed — within noise. A full solution build before and after produced **identical** diagnostics: ~~614 distinct (file, rule) pairs over 6,108 occurrences~~ — **re-measured under `MAJ-07` on the shipped tree as 641 distinct `(rule, file)` pairs over 6,056 emitted `CA` warning lines, reducing to 3,028 unique `(file, line, column, rule)` diagnostics** — 3,055 warnings, 0 errors. Re-measured again for `MAJ-01` with a far larger bound: `WebVella.Erp.Web` had still not finished past **2,700 seconds**. |
| **REMEDIATION** | **Closed, and extended under `MAJ-01`.** The suppression is now conditioned on `MSBuildProjectName` and applies to `WebVella.Erp.Web` alone, with both values exposed as readable properties that Gate 1 reads back and asserts per project. The positive control was flipped from *must not fire* to **must fire**, so the eighteen zeros are provably an absence of defects. Narrowing the exclusion left the nineteenth compilation unscanned, which `MAJ-01` treated as a coverage gap in its own right: a dedicated workflow step now scans that compilation with the family armed and the interprocedural chain bounded to one hop — about 220 seconds, exit 0, **zero** `CA3001`–`CA3012` — behind a capability self-test that requires `CA3001` and `CA3003` against deliberate flows, with a timeout treated as fatal and `taint-scan-web.txt` published as evidence. **Taint coverage is 19 of 19 compilations**; the residual is analysis depth for that one project (`RISK-051`). A new Gate 1 step asserts the render route still exists, carries seven exact authorization invariants, that the privilege check precedes component-type resolution by line number, and that the runtime-compilation inventory is unchanged — with comment lines excluded so a documentation edit cannot fail it. Three negative controls each fail the gate. |
