...
| Closed since this table was written | `AllowAnyOrigin()` is applied at **no** host. This row has been corrected twice, and both superseded readings are stated so the progression is auditable: it first said "two hosts", then "one host — `WebVella.Erp.Site.Project`". Both hosts have since been narrowed to the explicit origin allow-list each one's own commented-out policy documents — `WebVella.Erp.Site` in source, and `WebVella.Erp.Site.Project` through `Settings:Cors:AllowedOrigins`, which falls back to that documented list **only** in `Development` and denies every origin when unconfigured elsewhere. Verify with `git grep -n 'AllowAnyOrigin' -- '*.cs'`, which now returns explanatory comments only and no applied policy. `RISK-013` is closed in the [risk register](risk-register.md) |
