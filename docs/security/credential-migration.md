...
| **Not** part of this change | The seeded administrator credential and the 6-to-24-character password bounds in `WebVella.Erp/ERPService.cs` are **unchanged**, and no schema version 4 data migration has been added. Sections below that describe those steps describe the **planned** authorization and provisioning class, which has not landed. Treat them as the specification for that work, not as a description of this commit |
